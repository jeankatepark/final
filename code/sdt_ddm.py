...
file_path = "https://raw.githubusercontent.com/joachimvandekerckhove/cogs107s25/refs/heads/main/final/data.csv"
...
def apply_hierarchical_sdt_model(data_sdt):
    ...
    P = len(data_sdt['pnum'].unique())
    C = len(data_sdt['condition'].unique())
    ...
    with pm.Model() as sdt_model:
        ...
        pm.Binomial('hit_obs', 
                   n=data_sdt['nSignal'], 
                   p=hit_rate[data_sdt['pnum']-1, data_sdt['condition']], 
                   observed=data_sdt['hits'])
        
        # Likelihood for noise trials
        pm.Binomial('false_alarm_obs', 
                   n=data_sdt['nNoise'], 
                   p=false_alarm_rate[data_sdt['pnum']-1, data_sdt['condition']], 
                   observed=data_sdt['false_alarms'])
...
def draw_delta_plots(data_delta, pnum):
    ...
    data_delta = data_delta[data_delta['pnum'] == pnum]
    
    # Get unique conditions and create subplot matrix
    conditions = data_delta['condition'].unique()
    ...
    for i, cond1 in enumerate(conditions):
        for j, cond2 in enumerate(conditions):
            ...
            cmask1 = data_delta['condition'] == cond1
            cmask2 = data_delta['condition'] == cond2
            overall_mask = data_delta['mode'] == 'overall'
            error_mask = data_delta['mode'] == 'error'
            accurate_mask = data_delta['mode'] == 'accurate'
            
            # Calculate RT differences for overall performance
            quantiles1 = [data_delta[cmask1 & overall_mask][f'p{p}'] for p in PERCENTILES]
            quantiles2 = [data_delta[cmask2 & overall_mask][f'p{p}'] for p in PERCENTILES]
            overall_delta = np.array(quantiles2) - np.array(quantiles1)
            
            # Calculate RT differences for error responses
            error_quantiles1 = [data_delta[cmask1 & error_mask][f'p{p}'] for p in PERCENTILES]
            error_quantiles2 = [data_delta[cmask2 & error_mask][f'p{p}'] for p in PERCENTILES]
            error_delta = np.array(error_quantiles2) - np.array(error_quantiles1)
            
            # Calculate RT differences for accurate responses
            accurate_quantiles1 = [data_delta[cmask1 & accurate_mask][f'p{p}'] for p in PERCENTILES]
            accurate_quantiles2 = [data_delta[cmask2 & accurate_mask][f'p{p}'] for p in PERCENTILES]
